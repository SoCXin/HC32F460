...
static en_result_t PWC_enNvicBackup(void)
{
    ...
    for (u8Cnt = 0u; u8Cnt < sizeof(PWC_NVIC_ISER_BAK)/sizeof(uint32_t); u8Cnt++)
    {
        PWC_NVIC_ISER_BAK[u8Cnt] = NVIC->ISER[u8Cnt];
    }
...
static en_result_t PWC_enNvicRecover(void)
{
    ...
    for (u8Cnt = 0u; u8Cnt < sizeof(PWC_NVIC_ISER_BAK)/sizeof(uint32_t); u8Cnt++)
    {
        NVIC->ISER[u8Cnt] = PWC_NVIC_ISER_BAK[u8Cnt];
    }
    return Ok;
}
